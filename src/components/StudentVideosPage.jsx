...
function isPlayableVideoLink(link) {
    const value = String(link || '').split('?')[0].toLowerCase();
    return /(\.mp4|\.webm|\.ogg|\.mov|\.m4v|\.mkv|\.avi)$/.test(value);
}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="relative w-full sm:w-80">
                    ...
                    <input
                        ...
                        placeholder="Mavzu, guruh yoki ustoz bo'yicha qidirish"
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredVideos.map((video) => {
                        ...
                        const canPreview = attachment.link && isPlayableVideoLink(attachment.link);

                        return (
                            <article key={video.id} className="bg-white rounded-2xl border border-[#dce1ea] p-5 shadow-sm">
                                <div className="w-11 h-11 rounded-xl bg-[#fff2e4] text-[#be7734] flex items-center justify-center mb-3">
                                    <PlayCircle size={20} />
                                </div>

                                <h3 className="text-base font-semibold text-gray-900 line-clamp-2">
                                    {video.lesson?.title || 'Dars videosi'}
                                </h3>
