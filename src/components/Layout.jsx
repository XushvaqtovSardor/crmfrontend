...
import {
  Home, Users, GraduationCap, Layers3, BookOpen, DoorOpen, Wallet, Gift,
  Settings, Bell, Moon, Sun, Search, LogOut, Menu, X, ChevronDown, CalendarDays, Plus,
  ChevronLeft, ChevronRight,
} from 'lucide-react';
import { useAuth } from '../AuthContext.jsx';
import { useNavigate, useLocation } from 'react-router-dom';

// Role-based navigation map used to render the sidebar menu.
const SIDEBAR_ITEMS = {
  SUPERADMIN: [
    ...
    { name: "Sovg'alar", icon: Gift, path: '/courses' },
  ...
  ADMIN: [
    ...
    { name: 'Talabalar', icon: GraduationCap, path: '/students' },
    { name: "Sovg'alar", icon: Gift, path: '/courses' },
    { name: 'Moliya', icon: Wallet, path: '/rooms' },
    { name: 'Boshqarish', icon: Settings, path: '/settings' },
  ],
  TEACHER: [
    { name: 'Asosiy', icon: Home, path: '/dashboard' },
...
export default function Layout({ children }) {
  ...
  const toggleTheme = () => {
    ...
    if (next) document.body.classList.add('theme-night');
      ...
      `}>
        ...
        <div className="relative h-16 flex items-center justify-between px-4 border-b" style={{ borderColor: 'var(--border)' }}>
          ...
          <button
            ...
            className="hidden lg:flex w-8 h-8 rounded-xl items-center justify-center transition text-white"
        ...
        <div className="relative p-3 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          <button
            ...
            className={`mt-2 w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-red-500 hover:bg-red-50 transition ${!sidebarOpen ? 'justify-center' : ''}`}
          >
        ...
        <header className="h-16 border-b flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30 backdrop-blur-lg" style={{ background: 'color-mix(in srgb, var(--surface) 94%, transparent)', borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                ...
                className="w-60 lg:w-72 rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2"
                style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text)', boxShadow: '0 0 0 0 var(--ring)' }}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button className="hidden lg:flex h-10 px-4 rounded-xl border items-center gap-2 text-sm text-gray-600" style={{ borderColor: 'var(--border)', background: '#fff' }}>
              AICoder markazi <ChevronDown size={16} />
            </button>
            <button className="hidden lg:flex h-10 px-4 rounded-xl border items-center gap-2 text-sm text-gray-600" style={{ borderColor: 'var(--border)', background: '#fff' }}>
            ...
            <button
              onClick={toggleTheme}
              className="w-10 h-10 rounded-xl text-white flex items-center justify-center transition"
